...
key the client holds, so a stolen token is useless to anyone without that key. See
...
- DPoP requirements (`PRD-3.md`) and plan (`PLAN-3.md`), both marked complete. A
  README DPoP + BFF section (the endpoint table under the `DPoP` scheme, the browser
  walkthrough), a `CLAUDE.md` iteration pointer and DPoP architecture note, and
  planning docs for future iterations (`PRD-4.md` distributed replay protection,
  `PRD-5.md` browser-key SPA).

## [0.2.0] - 2026-07-05

**Step-up authentication.** A higher-assurance level (`pro`) so a more sensitive
endpoint can demand a stronger proof of identity than a plain password session. See
[`PRD-2.md`](PRD-2.md) / [`PLAN-2.md`](PLAN-2.md).
...
- Step-up requirements (`PRD-2.md`) and plan (`PLAN-2.md`), both marked complete. A
  README step-up section (levels, the seed TOTP note, the elevated endpoint), a
  `CLAUDE.md` iteration pointer and step-up architecture note.
